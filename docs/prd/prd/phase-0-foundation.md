...
> 说明：目标 1/2/3 为「引擎能力」，已全部落地；目标 4/5/6 为「AI 能力」，其**代码层（Copilot 意图识别+代码生成、GeoAI 空间化入图、NLPG 文本→SQL）已随 Phase 0 落地**（对应 D5/D6/D7 ✅，详见 §5.5/§5.6/§5.7）。端到端「对话/联调」需接入 LLM 接口（swagger）后方完整，故标 🟡，随 Phase 1 协同完成联调。
...
| # | 意图 | 输入示例 | 输出 | 状态 |
|---|------|---------|------|------|
| I-1 | 创建基础地图 | "创建一个武汉地图，暗色主题，缩放 12" | Map 初始化代码 | ✅ 已落地（`packages/ai copilot` 的 `create_map` 意图 + `generateCode`） |
| I-2 | 添加点标记 | "在光谷添加一个红色标记点" | addLayer + Marker 代码 | ✅ 已落地（`add_marker` 意图） |
| I-3 | 添加线/面 | "画一条从汉口到武昌的蓝色路线" | addLayer + LineString 代码 | ✅ 已落地（`add_line_polygon` 意图） |
| I-4 | 数据可视化 | "把这些 POI 按热度做成热力图" | Heatmap 图层代码 | ✅ 已落地（`heatmap` 意图，见 `copilot.test.ts`） |
| I-5 | 交互功能 | "点击标记弹出信息窗" | popup + event 代码 | ✅ 已落地（`popup_interaction` 意图，生成 `map.on('click',...)`） |
...
| # | 功能 | 描述 | 状态 |
|---|------|------|------|
| G-1 | 表头自动识别 | 上传文件后自动识别地址列、名称列、分类列 | ✅ 已落地（`geoai/headerDetector` 的 `detectHeaders`） |
| G-2 | 地址解析 | 支持中文地址（含错别字、口语化表达）→ 标准化地址 | ✅ 已落地（`geoai/addressParser` 的 `parseAddress`，覆盖 POI/简称/口语化） |
| G-3 | 坐标系判断 | 自动判断数据的坐标系（GCJ-02/WGS84/CGCS2000） | ✅ 已落地（`geoai/crsDetector` 的 `detectCRS`，含抽样已有坐标纠偏） |
| G-4 | 批量地理编码 | 10 万条地址可在 5 分钟内完成编码 | ✅ 已落地（`geoai/geocoder` 的 `batchGeocode`，本地库优先；速率待压测） |
| G-5 | 地图预览 | 编码完成后直接在地图上预览结果 | ✅ 已落地（`geoai/dataImporter` 的 `importToGeoJSON` 导出 GeoJSON 供预览） |
...
| W7 | Copilot v0.1 | 3 类意图支持 | ✅ 已实现 5 类意图（含 v0.1 的 3 类） |
| W8 | Copilot v1 | 5 类意图 + 沙箱运行 | 🟡 5 类意图代码生成已落地（`copilot` 模块），沙箱运行待 LLM 联调 |
| W9 | GeoAI v0.1 | CSV 地址解析 | ✅ 已落地（`geoai` 模块：表头识别+地址解析+坐标系判断+批量编码） |
| W10 | NLPG v0.1 | 基础查询 + SQL 校验 | ✅ 已落地（`nlpg` 模块：文本→SQL + `validateSql` 校验层） |
| W11 | Demo v1 | 基础地图演示上线 | ✅ |
| W12 | Phase 0 RC | 所有模块联调 + 文档完善 | 🟡（引擎/展示/文档✅，AI 三模块代码层✅、端到端联调🟡） |

> **进度小结**：W1-W6、W9、W10、W11 已完成；W7 已完成（超额实现 5 类意图）；W8 代码层完成、沙箱运行待联调；W12 部分完成。Phase 0 的「引擎 + 展示 + 文档 + AI 代码层」闭环已打通，AI 能力的端到端联调（LLM/swagger）为 Phase 1 核心待办。
...
| # | 风险 | 影响 | 缓解措施 |
|---|------|------|----------|
| R1 | **AI 三模块（Copilot/NLPG/GeoAI）代码层已落地，但端到端联调未完成即对外宣称「可用」** | 客户/决策方高估成熟度，交付承诺失实 | 全文档 ✅/🟡/❌ 状态标注（代码层✅、端到端联调🟡）；对外材料仅引用 ✅ 清单，联调能力明确标注 🟡 |
