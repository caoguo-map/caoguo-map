...
| # | 功能 | 描述 | 优先级 |
|---|------|------|--------|
| P-1 | 拓扑渲染 | 管段按类型着色（燃气=黄色/供水=蓝色/供热=红色），节点按设备类型显示图标 | P0 |
| P-2 | 层级钻取 | 区域→街道→小区→楼栋，逐级钻取管网 | P0 |
| P-3 | 设备卡片 | 点击节点/管段弹出信息卡片（属性+图片+维护记录） | P0 |
| P-4 | 连通性高亮 | 选中某段管线，高亮显示上下游连通路径 | P1 |
| P-5 | 拓扑编辑 | 拖拽添加管段/节点，自动维护连通性 | P2 |
| P-6 | 搜索定位 | 按设备编号/地址/区域搜索并定位 | P1 |
| P-7 | 分层控制 | 按管径/材质/年代/状态分层显示 | P1 |
...
| # | 功能 | 描述 | 优先级 |
|---|------|------|--------|
| B-1 | 一键推演 | 点击故障管段，自动触发推演 | P0 |
| B-2 | 影响范围可视化 | 受影响区域用半透明红色多边形高亮 | P0 |
| B-3 | 受影响列表 | 侧边栏展示受影响用户/建筑列表 | P0 |
| B-4 | 阀门隔离方案 | 展示需要关闭/打开的阀门 + 操作顺序 | P0 |
| B-5 | 重要用户标注 | 医院/学校/工厂等重要用户用特殊标记 | P1 |
| B-6 | 备选供气/供水路径 | 如有备用管线，推荐替代供应方案 | P1 |
| B-7 | 推演历史 | 保存推演记录，可回溯对比 | P2 |
...
| # | 功能 | 优先级 |
|---|------|--------|
| L-1 | 参数面板：风向/风速/泄漏量可调节 | P0 |
| L-2 | 动态扩散动画：危险区域随时间扩大 | P0 |
| L-3 | 等浓度线：展示不同浓度等级的危险区域 | P1 |
| L-4 | 叠加分析：危险区域与建筑/人口数据叠加 | P1 |
| L-5 | 气象数据接入：自动获取实时风向/风速 | P2 |
...
|---|------|--------|
| H-1 | 健康评分计算引擎 | P0 |
| H-2 | 风险热力图（按区域聚合显示） | P0 |
| H-3 | 单管线健康详情（各维度评分+雷达图） | P1 |
| H-4 | 优先维护建议列表（风险最高的 Top N 管线） | P1 |
| H-5 | 评分模型参数可配置（权重/阈值可调） | P2 |
