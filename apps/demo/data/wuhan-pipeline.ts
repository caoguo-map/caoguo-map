/**
 * 武汉模拟管网数据（Phase 1 管网推演演示用）
 * - 拓扑数据集：节点（含阀门/水表/制水厂）+ 管段（PE/球墨铸铁）
 * - 用户：含学校（重要用户）+ 居民
 *
 * 注意：合成数据，仅用于演示。
 */
...
export const wuhanPipeline: PipelineTopologyDataset = {
  nodes: [
    // 制水厂
    { id: 'plant-east', kind: 'source', lng: 114.42, lat: 30.51, pipelineType: 'water', properties: { capacity: 2000, code: 'EAST' } },
    { id: 'plant-west', kind: 'source', lng: 114.22, lat: 30.55, pipelineType: 'water', properties: { capacity: 1500, code: 'WEST' } },

    // 主干管阀门
    { id: 'v01', kind: 'valve', lng: 114.40, lat: 30.52, pipelineType: 'water', properties: { valveStatus: 'open', code: 'V01' } },
    ...
    { id: 'v04', kind: 'valve', lng: 114.30, lat: 30.56, pipelineType: 'water', properties: { valveStatus: 'open', code: 'V04' } },
    ...
    { id: 'v07', kind: 'valve', lng: 114.32, lat: 30.50, pipelineType: 'water', properties: { valveStatus: 'open', code: 'V07' } },
    ...
    // 储水罐
    { id: 'tank-1', kind: 'tank', lng: 114.30, lat: 30.54, pipelineType: 'water' },
  ],
  pipes: [
    ...
    { id: 'p02', fromNode: 'v01', toNode: 'v06', type: 'pipe', pipelineType: 'water', properties: { diameter: 1000, material: 'ductile_iron', installDate: '2010-06-01', pressure: 0.38, ratedPressure: 0.6, status: 'normal' } },
...
/** 在拓扑中找一根管段 ID 的快捷索引 */
export const pipeIds = wuhanPipeline.pipes.map((p) => p.id);
